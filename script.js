...
// Toggle dropdown visibility and show dashboard if logged in
accountBtn.addEventListener("click", function (e) {
  ...
  if (getUser) {
    ...
    UserName.innerHTML = `${getUser?.firstName || ""} ${
      getUser?.lastName || ""
    }`;
...
loginBtn.addEventListener("click", async function () {
  ...
  const emailSchema = /^[a-z0-9._%+-]{1,50}@[a-z0-9.-]{1,50}\.[a-z]{2,}$/i;
...
signupBtn.addEventListener("click", async function () {
  ...
  if (!fullNameSchema.test(name)) {
    error = "Full name must be 3–20 characters (letters and spaces only).";
